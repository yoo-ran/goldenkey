  ...
  return (
    <section className='w-full md:w-full relative  z-40 flexCol my-6'>
      <div className={`relative w-full  top-14  flexCol gap-y-4`}>
        <article
          className={`absolute z-50 top-0 md:left-0 bg-primary border-2 rounded-md drop-shadow p-4 md:p-0 md:pb-4  mobile_6 w-full md:w-2/12 transition-[all]  ${
            openFilter ? 'translate-y-1.5' : 'hidden'
          }`}
        >
          <div className='mobile_4_bold border-b w-full text-center text-primary-yellow pb-4 md:py-4'>
            <p>거주유형선택</p>
          </div>
          <div className='w-full  md:pl-4'>
            ...
            <div className='grid grid-cols-3 md:grid-cols-1 md:grid-rows-3 gap-x-4 gap-y-8 mt-3'>
              <button
                onClick={() =>
                  filterType !== 'house/villa'
                    ? setFilterType('house/villa')
                    : setFilterType('')
                }
                className={`w-full  py-4 mobile_5 rounded-md md:rounded-r-none ${
                  filterType === 'house/villa'
                    ? 'bg-secondary-yellow'
                    : 'bg-secondary-light '
                }`}
              ...
              <button
                onClick={() =>
                  filterType !== 'oneroom/tworoom'
                    ? setFilterType('oneroom/tworoom')
                    : setFilterType('')
                }
                className={`bg-secondary-light w-full py-4 mobile_5  rounded-md md:rounded-r-none ${
                  filterType === 'oneroom/tworoom'
                    ? 'bg-secondary-yellow'
                    : 'bg-secondary-light '
                }`}
        ...
        <article
          className={`absolute flexCol top-44 md:top-2 z-40 md:right-0 border w-full md:w-10/12 py-8 bg-white transition-all overflow-hidden transform ${
            filterType === '' || openFilter === false ? 'scale-y-0' : ''
          }`}
        >
          {filterPropertyType()}
        </article>
